...
router.post("/new", async (req, res) => {
  ...
  try {
    ...
    if (!chatSubscription) {
      ...
      res.json({
        message: `Вы успешно подписались на уведомления по подписке ${subName}. Настройки бота обновятся в течение 2 часов.`,
      });
    }
  } catch (error) {
...
router.delete("/delete", async (req, res) => {
  ...
  try {
    ...
    const message =
      deleteResult > 0
        ? "Подписка была удалена.\nНастройки бота обновятся в течение 2 часов."
